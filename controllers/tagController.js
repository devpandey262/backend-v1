...
module.exports.getAllTagsOfType = async (req, res, next) => {
  ...
  try {
    const resp = await Tag.find({ type: tagType }, { tag: 1, _id: 0 })
      .sort({ count: -1 })
      .limit(10)
      .skip(offset);
...
module.exports.addUserDefinedTag = async (req, res, next) => {
  ...
  try {
    ...
    let problemTags = await Question.findById(problemId, { _id: 0, tags: 1 });
    ...
    if (user_id !== null) {
      if (value === undefined) {
        ...
        userData = await User.aggregate([
          ...
          {
            $match: {
              $and: [{ _id: ObjectID(user_id) }, { tags: { $regex: value } }],
            },
          },
          { $group: { _id: { tags: "$tags" } } },
        ]).limit(10);
...
module.exports.getUserDefinedTags = async (req, res) => {
  ...
  try {
    ...
    res.status(500).send({
      err: "Error fetching user defined tags",
    });
  }
};
